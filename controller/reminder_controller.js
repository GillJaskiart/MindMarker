...
let remindersController = {
  list: (req, res, user) => {
    console.log("req.user for /reminders redirection from dashboard: ",  req.user)
    // userID = req.user.id
    // console.log(database.database[userID - 1].reminders)

    res.render("reminder/index", { reminders: database.database[req.user.id - 1].reminders });
  },
  ...
  create: (req, res) => {
    let reminder = {
      id: (database.database[req.user.id - 1].reminders).length + 1,
      title: req.body.title,
      description: req.body.description,
      completed: false,
    };
    database.database[req.user.id - 1].reminders.push(reminder);
  ...
  update: (req, res) => {
    ...
    const reminderToEdit = database.database[req.user.id - 1].reminders.find( (reminder) => reminder.id === reminderId)
    if (reminderToEdit) {
  ...
  delete: (req, res) => {
    ...
    const reminderToDelete = database.database[req.user.id - 1].reminders.find( (reminder) => reminder.id === reminderId)
    if (reminderToDelete) {
